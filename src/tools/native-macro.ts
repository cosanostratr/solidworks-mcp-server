 ...
 */
const macroRecordingResultSchema = z.object({
  success: z.boolean(),
  message: z.string(),
  macroPath: z.string().optional(),
  status: z.enum(['recording', 'paused']).optional(),
  options: z.object({
    recordViewCommands: z.boolean().optional(),
    recordFeatureManager: z.boolean().optional(),
    recordSelections: z.boolean().optional(),
  }).optional(),
});
...
export const nativeMacroTools = [
  ...
  
  {
    ...
    outputSchema: macroRecordingResultSchema,
    handler: (args: any, swApi: SolidWorksAPI) => {
      try {
        ...
        return {
          success: false,
          message: `Failed to start native macro recording: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    }
  },

  {
    ...
    outputSchema: z.object({
      ...
      message: z.string(),
      macroPath: z.string().optional(),
      openedInEditor: z.boolean().optional(),
    ...
    handler: (args: any, swApi: SolidWorksAPI) => {
      try {
        ...
        return {
          success: false,
          message: `Failed to stop native macro recording: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    }
  },

  {
    ...
    outputSchema: z.object({
      success: z.boolean(),
      message: z.string(),
      status: z.enum(['paused', 'recording']).optional(),
    }),
    handler: (args: any, swApi: SolidWorksAPI) => {
      try {
        const swApp = swApi.getApp();
        ...
        return {
          success: false,
          message: `Failed to ${args.action} macro recording: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    }
  },
  ...
  
  {
    ...
    outputSchema: z.object({
      ...
      hadArguments: z.number().optional(),
    }),
    handler: (args: any, swApi: SolidWorksAPI) => {
      try {
        const swApp = swApi.getApp();
        ...
        if (args.arguments && args.arguments.length > 0) {
          ...
          const vbaApp = swApp.GetAddInObject?.("SldWorks.MacroRunner") as { RunMacroWithArguments(path: string, module: string, proc: string, args: unknown[]): unknown } | null;
        ...
        return {
          success: false,
          message: `Failed to run macro: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    }
  },

  {
    ...
    handler: (args: any, swApi: SolidWorksAPI) => {
      try {
        ...
        return {
          success: false,
          message: `Failed to edit macro: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    }
  },
  ...
  
  {
    ...
    handler: (args: any, swApi: SolidWorksAPI) => {
      try {
        ...
        if (typeof (swApp as any).RecordMacro !== 'function') {
          throw new Error(
            'SolidWorks API does not expose RecordMacro() in this environment. ' +
            'Native macro initialization is not available; please use text-based VBA ' +
            'generation tools (e.g. vba_* tools) instead.'
          );
        }
        ...
        (swApp as any).RecordMacro(macroPath);
        ...
        // Get VBA environment
        interface VBACodeModule {
          CountOfLines: number;
          DeleteLines(start: number, count: number): void;
          InsertLines(line: number, code: string): void;
        }
        interface VBAProject {
          VBComponents(name: string): { CodeModule: VBACodeModule };
          Save(): void;
          References: unknown;
        }
        const vbaEditor = swApp.GetAddInObject?.("VBA.Editor") as { ActiveVBProject?: VBAProject } | null;
        const vbaProject = vbaEditor?.ActiveVBProject;

        if (!vbaProject) {
          throw new Error('VBA project not available');
        }
        ...
        return {
          success: false,
          message: `Failed to create initialized macro: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    }
  },

  {
    ...
    outputSchema: z.object({
      ...
      macroName: z.string().optional(),
      linesOfCode: z.number().optional(),
      referencesAdded: z.boolean().optional(),
      initializationAdded: z.boolean().optional(),
    }),
    handler: (args: any, swApi: SolidWorksAPI) => {
      try {
        const swApp = swApi.getApp();
        ...
        if (typeof (swApp as any).RecordMacro !== 'function') {
          throw new Error(
            'SolidWorks API does not expose RecordMacro() in this environment. ' +
            'Cannot auto-initialize native macro; save a starter .swp from SolidWorks ' +
            'and pass it via outputPath instead.'
          );
        }
        (swApp as any).RecordMacro(outputPath);
        (swApp as any).StopMacroRecording();
        ...
        interface VBAReferences {
          AddFromGuid(guid: string, major: number, minor: number): void;
          Item(index: number): unknown;
          Count: number;
        }
        interface VBAProject2 {
          References: VBAReferences;
          VBComponents(name: string): { CodeModule: VBACodeModule };
          Save(): void;
        }
        const vbaEditor = swApp.GetAddInObject?.("VBA.Editor") as { ActiveVBProject?: VBAProject2 } | null;
        const vbaProject = vbaEditor?.ActiveVBProject;
...
End Sub`;
        ...
        return {
          success: false,
          message: `Failed to convert text to native macro: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    }
  },
  ...
  
  {
    ...
    handler: async (args: any, swApi: SolidWorksAPI) => {
      try {
        ...
        return {
          success: false,
          message: `Failed to run batch macros: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    }
  }
