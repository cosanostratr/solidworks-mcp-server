...
const compileTemplate = (templateName: string): any => {
  ...
  const templateMap: Record<string, string> = {
    'batch_export': 'batch_process', // Unified batch processing template
    ...
    handler: async (args: any, swApi: SolidWorksAPI) => {
      try {
        ...
        if (args.outputPath) {
          ...
          return {
            success: true,
            message: `VBA script generated and saved to: ${args.outputPath}`,
            vbaCode,
        ...
        return {
          success: true,
          message: 'VBA script generated successfully',
          vbaCode,
          linesOfCode: vbaCode.split('\n').length,
        };
      } catch (error) {
        return {
          success: false,
          message: `Failed to generate VBA script: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    },
  },
...
End Sub`,
      ...
      const vbaCode = vbaTemplates[args.featureType] || 'Feature type not yet implemented';
      return {
        success: vbaCode !== 'Feature type not yet implemented',
        message: vbaCode !== 'Feature type not yet implemented' ? 'VBA code generated successfully' : 'Feature type not yet implemented',
        vbaCode: vbaCode !== 'Feature type not yet implemented' ? vbaCode : undefined,
        linesOfCode: vbaCode !== 'Feature type not yet implemented' ? vbaCode.split('\n').length : undefined,
      };
    },
  },
  
  {
    ...
    handler: (args: any, swApi: SolidWorksAPI) => {
      try {
        ...
        return {
          success: true,
          message: 'Batch VBA code generated successfully',
          vbaCode,
          linesOfCode: vbaCode.split('\n').length,
        };
      } catch (error) {
        return {
          success: false,
          message: `Failed to generate batch VBA: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    },
  },
  
  {
    ...
    handler: (args: any, swApi: SolidWorksAPI) => {
      try {
        ...
        return {
          success: false,
          message: `Failed to execute macro: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    },
  },
  
  {
    ...
    handler: (args: any, swApi: SolidWorksAPI) => {
      try {
        ...
        return {
          success: true,
          message: 'Drawing VBA code generated successfully',
          vbaCode,
          linesOfCode: vbaCode.split('\n').length,
        };
      } catch (error) {
        return {
          success: false,
          message: `Failed to generate drawing VBA: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    },
  },
