...
Sub ManageDrawingSheet_${args.operation}()
    ...
    If Not swSheet Is Nothing Then
        ...
        ' Update size
        ' NOTE: Sheet.SetSize() does NOT exist in SolidWorks API
        ' Use DrawingDoc.SetupSheet5() instead (see other VBA examples)
        ${args.sheetSize ? `
        ' Sheet.SetSize is not available - use SetupSheet5 in DrawingDoc
        ' This requires more parameters, so we'll use SetScale as fallback
        ' For full sheet setup, use DrawingDoc.SetupSheet5() method` : ''}
