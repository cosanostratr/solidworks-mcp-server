...
End Sub`
      ...
      const vbaCode = templates[args.geometryType] || 'Geometry type not supported';
      return {
        success: vbaCode !== 'Geometry type not supported',
        message: vbaCode !== 'Geometry type not supported' ? 'VBA code generated successfully' : 'Geometry type not supported',
        vbaCode: vbaCode !== 'Geometry type not supported' ? vbaCode : undefined,
        linesOfCode: vbaCode !== 'Geometry type not supported' ? vbaCode.split('\n').length : undefined,
      };
    }
  },
...
Sub CreateSweepFeature()
    ...
    ' Select profile
    ' NOTE: 'Sketch1' and 'Sketch2' are default fallback values
    ' In production, these should be replaced with actual sketch names from parameters
    swModel.Extension.SelectByID2 "${args.profiles[0] || 'Sketch1'}", "SKETCH", 0, 0, 0, False, 1, Nothing, 0
    
    ' Select path
    ' NOTE: Default 'Sketch2' is a fallback - use actual sketch name from parameters
...
End Sub`
      ...
      const vbaCode = templates[args.featureType] || 'Feature type not supported';
      return {
        success: vbaCode !== 'Feature type not supported',
        message: vbaCode !== 'Feature type not supported' ? 'VBA code generated successfully' : 'Feature type not supported',
        vbaCode: vbaCode !== 'Feature type not supported' ? vbaCode : undefined,
        linesOfCode: vbaCode !== 'Feature type not supported' ? vbaCode.split('\n').length : undefined,
      };
    }
  },
...
End Sub`
      ...
      const vbaCode = templates[args.patternType] || 'Pattern type not supported';
      return {
        success: vbaCode !== 'Pattern type not supported',
        message: vbaCode !== 'Pattern type not supported' ? 'VBA code generated successfully' : 'Pattern type not supported',
        vbaCode: vbaCode !== 'Pattern type not supported' ? vbaCode : undefined,
        linesOfCode: vbaCode !== 'Pattern type not supported' ? vbaCode.split('\n').length : undefined,
      };
    }
  },
...
End Sub`;
      return {
        success: true,
        message: 'Sheet metal VBA code generated successfully',
        vbaCode,
...
End Sub`;
      return {
        success: true,
        message: 'Surface modeling VBA code generated successfully',
        vbaCode,
        linesOfCode: vbaCode.split('\n').length,
      };
    }
